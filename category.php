<?php include('header.php'); 
  ...
  user ON blog.author_id=user.user_id WHERE cat_id = '$id' ORDER BY blog.publish_date DESC";
				...
				<div class="card-body d-flex blog_flex">
					<div class="flex-part1">
						<a href="single_post.php?id=<?=$result['blog_id']?>"> 
                            <?php $img = str_replace(' ','',$result['blog_image']);?>
                            <img src="admin/upload/<?=$img;?>"> </a>
					</div>
					<div class="flex-grow-1 flex-part2">
						  <a href="" id="title"><?= ucfirst($result['blog_title']);?> </a>
						<p>
						...
						<ul>
							<li class="me-2"><a href="single_post.php?id=<?=$result['blog_id']?>"> <span>
                                <i class="fa fa-pencil-square-o" aria-hidden="true"></i></span><?=$result['username'];?> </a>
							</li>
							<li class="me-2">
								<a href="single_post.php?id=<?=$result['blog_id']?>"> <span><i class="fa fa-calendar-o" aria-hidden="true"></i></span> <?=$result['publish_date'];?> </a>
							</li>
							<li>
								<a href="single_post.php?id=<?=$result['blog_id']?>"> <span><i class="fa fa-tag" aria-hidden="true"></i></span> <?=$result['cat_name'];?> </a>
						    </li>
						</ul>
					</div>
				</div>
